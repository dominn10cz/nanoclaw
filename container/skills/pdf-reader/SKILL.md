---
name: pdf-reader
description: Read and extract text from PDF files — documents, reports, contracts, spreadsheets. Use whenever you need to read PDF content, not just when explicitly asked. Handles local files and URLs.
...
If the channel adapter saved a PDF the user sent, its path will be present in the message text — typically under `attachments/` in the current group workspace. Use the path verbatim:

```bash
pdf-reader extract attachments/document.pdf --layout
```

If the user sent a link to a PDF instead of an attachment, prefer `pdf-reader fetch <url>` over downloading manually.
...
- pdftotext only extracts text-based PDFs. Scanned/image PDFs return empty output — fall back to `agent-browser` to render the file visually.
